...
def main():
    print("Emulando controlador PS4 - Probando eje Y del Giroscopio (supuesto Yaw / Viraje)")
    pad = vg.VDS4Gamepad()
    ...
    print("Enviando secuencias FIJAS a wGyroY usando el OFFSET 12 CORRECTO.")
    try:
        ...
        while True:
            ...
            # wGyroY está en bytes (14, 15) dentro de los 12 bytes del IMU en offset 12.
            packed_data = struct.pack("<hhhhhh", 0, value, 0, 0, 8192, 0)
            ctypes.memmove(ctypes.addressof(report_ex.Report) + 12, packed_data, 12)
            
            pad.update_extended_report(report_ex)
            pad.update()
            
            print(f"Enviando wGyroX = 0 | wGyroY (Yaw) = {value:>5} | wGyroZ = 0")
            time.sleep(1.0)
